    ...
    <header className="sticky top-0 z-50 border-b border-border bg-surface/90 backdrop-blur">
      <nav className="mx-auto flex max-w-3xl items-center justify-between px-4 py-3 sm:px-6">
        <a href="#top" className="group flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded bg-accent text-sm font-bold text-white transition-transform duration-200 group-hover:scale-110">
            JA
          </span>
          ...
          {LINKS.map((link) => (
            <li key={link.href}>
              <a
                ...
                className="block rounded-md px-2 py-3 text-sm font-medium text-muted transition-colors hover:bg-background hover:text-accent"
              >
                {link.label}
              </a>
            </li>
